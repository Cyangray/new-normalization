{
	...
	TH2F *h = new TH2F("h"," ",10,0.0,   7.680,10,1.630e-09,9.748e-06);
	ifstream strengthfile("strength.nrm");
	float strength[64],strengtherr[64],energyerr[64];
	float energy[631],trans[631];
	int i = 0;
   float a0 =  -0.7560;
   float a1 =   0.1280;
	float x;	
	while(strengthfile){
		...
		else{strengtherr[i-63] = x;}
		i++;
	}
	TGraphErrors *strengthexp = new TGraphErrors(64,energy,strength,energyerr,strengtherr);
    i = 0;
	...
	TLatex t;
	t.SetTextSize(0.05);
	t.DrawLatex(    1.436,2.924e-06,"^{xx}Yy");
	c1->Update();
